...
class ParagraphRenderer:
    def render(self, element: Dict[str, Any], indent_level: int = 0) -> List[str]:
        ...
        if "runs" in element and element["runs"]:
            ...
            if first_run_text.startswith("[Vidéo"):
                # Extraire le texte complet pour chercher l'ID
                full_text = "".join([run.get("text", "") for run in element["runs"]])
                print(f"DEBUG MARQUEUR VIDÉO: {full_text}")  # Pour déboguer
                ...
                if "video_id='" in full_text:
                    ...
                    if end_idx > start_idx:
                        ...
                        print(f"DEBUG ID VIDÉO: {video_id}")  # Pour déboguer
