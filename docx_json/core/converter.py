...
class DocxConverter:
    ...
    def __init__(self, docx_path: str, output_dir: str, save_images_to_disk: bool = True):
        """
        ...
        for rel in rels.values():
            if "image" in rel.target_ref:
                try:
                    ...
                    if rel.is_external:
                        logging.warning(
                            f"Image externe ignorée: {rel.target_ref}")
                        continue
                ...
                try:
                    ...
                    if image_name in self._images:
                        if self._save_images_to_disk:
                            ...
                            self._rels_dict[rel.rId] = f"data:image/png;base64,{self._images[image_name]}"
                except Exception as e:
                    logging.warning(
    ...
    def parse_paragraph(self, paragraph: DocxParagraph) -> DocumentElement:
        ...
        if text.startswith("[") and any(text.startswith(f"[{comp}") for comp in component_types):
            component_type = text[1:].split("]")[
                0] if "]" in text else "Unknown"
    ...
    def process_instructions(self, elements: List[DocumentElement]) -> List[DocumentElement]:
        """
        ...
        while i < len(elements):
            ...
            # Traitement des marqueurs de composants pédagogiques
                ...
                while j < len(elements):
                    ...
                    if next_element.type == "component_end" and next_element.component_type == component_type:
