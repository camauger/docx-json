...
def get_paragraph_json(paragraph: Paragraph) -> Dict[str, Any]:
        ...
        if hasattr(run, "_element"):
            ...
            if inline_elem is None:
                try:
                    ...
                    for child in run._element.iter("{*}inline"):
                        inline_elem = child
                        break
                except Exception:
                    logging.debug(
            ...
            if inline_elem is not None:
                ...
                try:
                    ...
                    for child in inline_elem.iter("{*}blip"):
                    ...
                    logging.debug(f"Impossible de récupérer l'ID de relation: {str(e)}")
    ...
    if style_name and style_name.startswith("Heading"):
        ...
        level = (
            ...
            if style_name and len(style_name.split(" ")) > 1
            else 1
        )
    ...
    if text.startswith("[") and any(
        ...
        return {"type": "component_marker", "component_type": component_type}

    # Vérifier si c'est un marqueur de fin de composant
    if text.startswith("[Fin ") and "]" in text:
        component_type = text[5:].split("]")[0]
        return {"type": "component_end", "component_type": component_type}
    ...
    while i < len(elements):
        ...
        # Traitement des marqueurs de composants pédagogiques
            ...
            while j < len(elements):
                ...
                if (
                    next_element.get("type") == "component_end"
                    and next_element.get("component_type") == component_type
                ):
            ...
            if component_type == "Vidéo":
                ...
                for content_elem in element["content"]:
                    if content_elem["type"] == "heading":
                        ...
                        content_html.extend(generate_element_html(content_elem))
                ...
                for i, content_elem in enumerate(element["content"]):
                    if content_elem["type"] == "heading":
                        ...
                        tab_contents[-1][2].extend(generate_element_html(content_elem))
